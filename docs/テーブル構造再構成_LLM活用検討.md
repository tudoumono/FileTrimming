...
- [x] **LLMのI/O設計**
  - 決定事項:
    ...
    - 初期適用範囲は「小規模な結合セルフォーム」だけに閉じず、元々の課題だった「2列 KV + メタヘッダー」候補も LLM 判定対象に含める。
    - 将来的に、複数テーブルが同一の意味ブロックを構成すると判断できる場合は、テーブル群単位の扱いを追加検討する。
...
2026-03-18 時点の採用版は、`src/llm/base.py` の dataclass 実装に合わせて **フラット構造** を採用する。初期検討で使っていた `source` / `sheet` / `table` のネスト案は、v1.0 実装では採用しない。
...
- 必須項目は観測事実のみで構成し、推定値は `hints` に分離する
- テーブル本体のセル構造は、既存の `CellData[][]` にできるだけ近い形を維持する
- 位置情報や周辺文脈は `context` に集約し、シート全体をそのまま抱え込まない
...
| `source_path` | `string` | 元ファイルの追跡パス |
| `source_ext` | `string` | 元ファイル拡張子 |
| `sheet_name` | `string` | Excel シート名。Word では `document` 相当を入れる |
| `table_caption` | `string` | 表キャプション |
| `rows` | `array[array[object]]` | 対象テーブルの観測事実 |

##### 任意フィールド

| フィールド | 型 | 内容 |
|---|---|---|
| `context` | `object` | 近接見出し・表プロファイル・元位置・前後文脈など |
| `hints` | `object` | `doc_role_guess` などの推定情報 |
...
```json
{
  ...
  "context": {
    ...
    "table_profile": {
      "non_empty_row_count": 5,
      "text_cell_count": 10,
      "max_text_cells_per_row": 2,
      "merged_text_cell_count": 0,
      "max_text_colspan": 1
    },
    "source_bounds": {
      "row_start": 1,
...
- `rows` は観測事実を保持する領域であり、セル本文を書き換えない
- `is_header` は抽出時点のヒントであり、解釈時には絶対視しない
- シート上の元位置は `context.source_bounds` で保持する
- シート名は `sheet_name` にフラットに置き、追加のネストは持たない
- `context.previous_table` は直前テーブルの列ラベル補完など、局所文脈の受け渡しに使う
- `hints` は省略可能とし、初期段階では最小限の推定情報だけを持てればよい
...
- `source` / `sheet` / `table` のネスト構造は検討段階で使った論理モデルであり、v1.0 実装では採用しない
- 必要な grouping は `source_*` / `sheet_name` / `table_caption` / `context.*` で表現する

##### 後続タスクへの受け渡し

- Task 3 では、この `ReconstructionUnit` を入力として受け取る LLM 解釈結果スキーマを定義する
- Task 4 では、`context.source_bounds` を既存中間表現にどう持たせるかを整理する
...
2026-03-18 時点の採用版は、`src/llm/base.py` の dataclass 実装に合わせて **最小限の v1.0** に揃える。初期検討で挙げていた `meta_header_rows` / `key_columns` / `value_columns` / `row_annotations` / `section_rows` は、将来拡張候補として残しつつ、v1.0 には採用しない。
...
- LLM 自己評価は `self_assessment` に入れてよいが、最終的なレビュー判定は別途レビュー用成果物で管理する
- 判定不能な場合でも `table_type = unknown` を返し、呼び出し側で既存解釈へ安全にフォールバックできる形を維持する
...
| `render_strategy` | `string` | Renderer に渡す出力戦略。v1.0 では `form_grid` / `key_value` / `data_table` |
...
| `column_labels` | `array[string]` | レンダリング時に使う確定ラベル |
| `active_columns` | `array[int]` | 実際に意味を持つ列候補 |
| `render_plan` | `object` | 行種別上書きや集計ラベル、Markdown 行差し替えなどの追加指示 |
| `notes` | `array[string]` | 補足メモ |
| `self_assessment` | `object` | LLM 自己評価 |

##### `render_plan` の v1.0 子項目

| フィールド | 型 | 内容 |
|---|---|---|
| `row_roles` | `array[string]` | 行ごとの描画方針上書き |
| `summary_labels` | `array[string]` | 1 行集計表に対する値ラベル |
| `markdown_lines` | `array[string]` | テーブル本文として採用したい Markdown 行 |
...
- `render_strategy`
  ...
  - `form` は概念上の類型であり、実際の描画戦略は `form_grid` を使う
- `header_rows`
  - 実データの列ラベルとして採用する行
- `data_start_row`
  - 2 列 KV のメタヘッダーを読み飛ばす場合などに使う
- `column_labels`
  - レンダリング時に使う確定ラベル
  - 多段ヘッダー時は LLM が最終ラベルを組み立てて返してよい
- `render_plan`
  - deterministic な既定挙動を壊しすぎない範囲で、行単位の補助指示を返す
- `self_assessment`
  - v1.0 では主に `confidence` を利用する

##### 設計上の注意

- LLM はセル本文を書き換えず、どの行/列をどう解釈したかを返す
- 判定に迷う場合は `table_type = unknown` を返せばよく、専用の `safe_unknown` 戦略は v1.0 では採用しない
- `notes` は省略可能であり、初期段階では短い補足のみを許容する
- `self_assessment.confidence` は `high` / `medium` / `low` の 3 段階とし、最終的なレビュー判定とは分離する
- `reason_codes` の標準化はレビュー用成果物側で扱い、`TableInterpretationResult` v1.0 には専用フィールドを増やさない
...
| 追加項目 | 追加先候補 | 理由 | 優先度 |
|---|---|---|---|
| テーブルのシート上 bounds | `TableElement` | `ReconstructionUnit.context.source_bounds` を生成するために必要 | 必須 |
...
| `sheet_name` を各テーブルに重複保持する項目 | `ReconstructionUnit.sheet_name` で保持するため、`TableElement` へは初期段階では追加しない |
...
- `rows` の構造自体は大きく変えない
- シート名は Excel のシート情報から `ReconstructionUnit.sheet_name` にそのまま格納する
...
- `llm_backend = openai | local | noop`
- `model_name`（backend ごとのモデル名）
- `prompt_version`
- `llm_observation_only = true | false`

##### 出力先イメージ

- `intermediate/03_transformed/<relative_path>.md`
- `intermediate/04_review/<relative_path>.llm_review.json`
- `intermediate/04_review/<relative_path>.llm_observation.json`
...
- 文書単位 JSON のトップレベルには `llm_backend` / `model_name` / `prompt_version` を残す
...
```json
{
  ...
  "prompt_version": "table_interpretation.v1",
  "unit_id": "input/excel/change_history.xlsx::設計概要::table_0001",
  "source_path": "input/excel/change_history.xlsx",
  "sheet_name": "設計概要",
  "table_id": "table_0001",
...
- 対象ファイル:
  ...
  - `src/extractors/excel.py`
- 目的:
  - `TableElement` にシート上の元位置を保持できるようにし、`ReconstructionUnit.context.source_bounds` の元情報を確保する
- 具体変更:
